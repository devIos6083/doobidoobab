...
| 날짜 | 작업 내용 |
|------|----------|
| **2025. 4. 2** | - 프로젝트 초기화 및 환경 세팅<br>- 사용자 정보 관리 서비스 구현 |
| **2025. 4. 5** | - 메인 화면 및 출석 체크 시스템 구현<br>- 그림 그리기 기능 개발 및 최적화 |
| **2025. 4. 10** | - **오늘의 게임 구현**: 여러 게임(이미지 매칭, 뜻 매칭, 단어 연결, 그림 그리기)을 랜덤하게 배치하여 진행<br>- 단어 학습 게임 로직 구현<br>- 각 게임의 UI 구성 및 애니메이션 적용<br>- 단어 연결 게임의 경우, 자동 연결에서 수동 연결 방식으로 전환하여 사용자 선택 기반으로 연결하도록 개선<br>- 코드 분량이 많아져 리펙토링의 필요성을 절감<br>- 그린 그림 저장 기능과 단어 연결 정답 판별 로직은 아직 미해결 상태 |
| **2025. 4. 13** | - 앱이 백그라운드 상태로 전환되었을 때, **오늘의 단어 학습을 하지 않은 경우 알림이 울리도록 구현**하여 사용자 동기부여 강화<br>- 전체 앱 구조를 **MVVM 아키텍처 패턴으로 전면 리팩토링**하여 유지보수성과 가독성 향상<br>- **오늘의 단어 게임에서 사용자가 그린 5개의 그림을 로컬에 저장**하고, 결과 화면에서 다시 보여주는 기능 구현<br>- **총 15개의 단어 중 사용자가 몇 개를 맞췄는지 통계 표시 기능** 추가<br>- **단어 연결 게임에서 연결은 되지만 다음 단계로 넘어가지 않던 버그 해결** |
| **2025. 4. 28** | - **Firebase Realtime Database로 백엔드 전환**: 로컬 SQLite에서 클라우드 기반 데이터베이스로 마이그레이션<br>- **데이터 구조화 및 안전성 강화**: 사용자 데이터, 게임 결과, 그림 데이터를 체계적으로 저장할 수 있는 구조 설계<br>- **데이터 마이그레이션 도구 개발**: 기존 SQLite 데이터를 Firebase로 안전하게 이전하는 유틸리티 구현<br>- **오프라인 지원 기능 추가**: 인터넷 연결이 없는 환경에서도 앱이 정상 작동하도록 오프라인 지원 구현<br>- **멀티 디바이스 동기화 지원**: 여러 기기에서 동일한 계정으로 접속 시 학습 데이터 동기화 기능 구현 |
| **2025. 5. 7** | - **그림 인식 AI 모델 통합 시작**: 사용자가 그린 그림을 분석하여 정확도를 평가하는 AI 시스템 개발 착수<br>- **Flask 백엔드 서버 구축**: 이미지 처리와 ML 모델 실행을 위한 Python 기반 백엔드 서버 구현<br>- **클라이언트-서버 통신 프로토콜 설계**: Flutter 앱과 Flask 서버 간 안정적인 통신을 위한 HTTP/REST API 설계<br>- **이미지 전처리 파이프라인 구현**: 그림 데이터를 AI 모델에 적합한 형식으로 변환하는 전처리 로직 개발<br>- **ngrok을 활용한 개발 환경 구축**: 로컬 개발 환경에서 테스트를 위한 터널링 솔루션 적용<br>- **임시 평가 알고리즘 구현**: 실제 ML 모델 통합 전 테스트를 위한 임시 평가 시스템 개발 |
